...
<li>Если вы собираетесь играть в аддоны-кампании, то <b>запускайте их только через Episode 2 VR</b>, поскольку они могут использовать контент Эпизодов.
...
<li>Не добавляйте больше одного аддона-кампании, чтобы избежать ошибок из-за файловых конфликтов.</li>
</ul>
...
<li>В некоторых аддонах-кампаниях отсутствует фоновая карта для меню, поэтому для навигации в меню придется использовать рабочий стол.</li>

<li>Если в моде не предусмотрено разделение на главы, первую карту придется загружать через консоль.</li>

<li>Если в моде есть какие-то нововведения в интерфейсе, скорее всего они не будут корректно работать (например радио-сообщения в моде Hatch18).</li>
...
<li>Некоторые моды могут вообще не запускаться. Для возможного исправления откройте папку аддона и удалите все файлы кроме папок, если они есть.
</li>
</ul>
